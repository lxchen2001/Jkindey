 ...
 */
public class PostAddPhotoAda extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    /**
     * 一行最多显示的图片数量
     */
    ...
    public PostAddPhotoAda(Context mContext, FragmentManager fragmentManager, List<PhotoInfo> mPhotoInfoList, XCallbackListener mXCallbackListener) {
        ...
        if (holder instanceof ContentViewHolder) {//显示图片数据
            ...
            if (position == getDatas().size() && position == NUM_ITEM) {
                ...
                ((FootViewHolder) holder).mImageAdd.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        ((ActPostAdd) mContext).openChoiceDialog(mContext, fragmentManager);
                    }
                });
            }
        }
